...
> 状态：**Phase 1-5 全部实施并实测通过（2026-08-15）**。
> Phase 5 新增：GoalFollow（持续跟随实体，目标走远自动恢复追击，精确距离判定 isReached）、followPath 显式路径执行（同高直线 waypoints）、/crpi fp follow + followpath 命令。
> 修复：GoalFollow 分支无条件 return 导致 executor.tick 被跳过（bot 完全不动的 bug）；block 坐标距离误判 SUCCESS 改为精确坐标。
