...
/**
 * One-block step up. Forward input + jump while grounded; the vanilla jump
 * physics carries the player onto the ledge.
 */
public final class MovementAscend implements Movement {
    private final PathNode source;
    private final PathNode target;
    private final int dx;
    private final int dz;

    public MovementAscend(PathNode source, PathNode target, int dx, int dz) {
        this.source = source;
        this.target = target;
        this.dx = dx;
        this.dz = dz;
    ...
    public void apply(FakePlayerHandle handle, FakePlayerMovementController controller, NavigationWorld world, long tick) {
        ...
        // jump only when grounded; the vanilla jump carries the player up
    ...
    public boolean isComplete(FakePlayerHandle handle, NavigationWorld world) {
        ...
        return handle.y() >= this.target.y() && dx * dx + dz * dz < 0.45 * 0.45;
    }

    @Override
    public String description() {
        return "Ascend " + (this.source.x() + this.dx) + "," + (this.source.y() + 1) + "," + (this.source.z() + this.dz);
    }
}
